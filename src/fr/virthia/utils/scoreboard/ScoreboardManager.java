...
public class ScoreboardManager {
    ...
    public ScoreboardManager() {
        ...
        glowingTask = Utils.INSTANCE.getScheduledExecutorService().scheduleAtFixedRate(() ->
        {
            String ip = colorIpAt();
            for (PersonalScoreboard scoreboard : scoreboards.values())
                Utils.INSTANCE.getExecutorMonoThread().execute(() -> scoreboard.setLines(ip));
        }, 80, 80, TimeUnit.MILLISECONDS);

        reloadingTask = Utils.INSTANCE.getScheduledExecutorService().scheduleAtFixedRate(() ->
        {
            for (PersonalScoreboard scoreboard : scoreboards.values())
                Utils.INSTANCE.getExecutorMonoThread().execute(scoreboard::reloadData);
