...
public class Displayer {
    ...
    public final ItemStack toItemStack() {
        if(getDisplayerType()== DisplayerType.SWITCHER) {
            ...
            if (isHead) {
                return new ItemCreator(getPrefix() + tmpText + getSuffix(), tmpValue,
                        "head").setHeadName(headPlayerName).setLores(getLore()).make();
            }
            return new ItemCreator(getPrefix() + tmpText + getSuffix(), getType(), tmpValue, tmpColor).setLores(getLore()).make();
            ...
            if (isHead) {
                return new ItemCreator(
                        getPrefix() + String.valueOf(((double) value) * multiplier) + getSuffix(),
                         tmpValue, "head").setLores(getLore()).setHeadName(getHeadPlayerName()).make();
            }
            return new ItemCreator(
                    getPrefix()+ String.valueOf(((double) value) * multiplier) + getSuffix(),
                    getType(), tmpValue, tmpColor).setLores(getLore()).make();
        }else if(getDisplayerType()== DisplayerType.DISPLAY_TEXT){
            if (isHead) {
                return new ItemCreator(getPrefix() + valueStr + getSuffix(), "head")
                        .setLores(getLore()).setHeadName(getHeadPlayerName()).make();
            }
            return new ItemCreator(getPrefix() + valueStr + getSuffix(), getType(), 1, color)
                    .setLores(getLore()).make();
        }
        return null;
    }
